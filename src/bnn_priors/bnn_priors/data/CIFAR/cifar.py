...
from .aug_data import CIFAR10Aug

__all__ = (
        'CIFAR10','CIFAR10_C', 'SVHN', 'CIFAR10Augmented', 'CIFAR10Small', 'CIFAR10Flips',
        'CIFAR10Crops', 'CIFAR10StackedAug',
)
...
class CIFAR10Augmented:
    def __init__(self, dtype='float32', device="cpu", download=True, augmentations="standard"):
        _ROOT = os.path.abspath(os.path.dirname(__file__))
        dataset_dir = f'{_ROOT}/cifar10/'
        ...
        if augmentations == "standard":
            ...
            print("using non standard augmentations")
            transform_train = transforms.Compose([
                *augmentations,
                transforms.ToTensor(),
                transforms.Normalize(X_mean_tuple, X_std_tuple),
...
class CIFAR10Flips(CIFAR10Augmented):
    def __init__(self, dtype='float32', device="cpu", download=True):
        augmentations = [transforms.RandomHorizontalFlip()]
        super().__init__(dtype=dtype, device=device, download=download, augmentations=augmentations)


class CIFAR10Crops(CIFAR10Augmented):
    def __init__(self, dtype='float32', device="cpu", download=True):
        augmentations = [transforms.RandomCrop(32, padding=4)]
        super().__init__(dtype=dtype, device=device, download=download, augmentations=augmentations)


class CIFAR10StackedAug(CIFAR10):
    def __init__(self, dtype='float32', device="cpu", download=True, n_aug=3, val_size=0.0):
        _ROOT = os.path.abspath(os.path.dirname(__file__))
        dataset_dir = f'{_ROOT}/cifar10/'
        dtype = getattr(t, dtype)
        self.dtype = dtype
        ...
        data_test = torchvision.datasets.CIFAR10(
            root=dataset_dir, download=download, train=False, transform=base_transform
        )
        data_train = CIFAR10Aug(
            root=dataset_dir, download=download, train=True, transform=aug_transform,
            base_transform=base_transform, n_aug=n_aug,
        )
        # TODO: hook up some validation set stuff

        self.norm = DatasetFromTorch(data_train, data_test, device=device)
        self.num_train_set = len(data_train)
        self.in_shape = t.Size([3, 32, 32])
        self.out_shape = [t.Size([n_aug, 3, 32, 32]), t.Size([])]
