...
def get_data(data: str, device: t.device):
    ...
    elif data == "cifar10_stackaug":
        dataset = bnn_priors.data.CIFAR10StackedAug(device=device)
    elif data == "cifar10_augmented":
        dataset = bnn_priors.data.CIFAR10Augmented(device=device)
    elif data == "cifar10_flips":
        dataset = bnn_priors.data.CIFAR10Flips(device=device)
    elif data == "cifar10_crops":
        dataset = bnn_priors.data.CIFAR10Crops(device=device)
...
def get_model(x_train, y_train, model, width, depth, weight_prior, weight_loc,
             ...
             weight_prior_params, bias_prior_params, softmax_temp, 
             label_smoothing, likelihood, consistency_std):
    print("temp: ", softmax_temp, "likelihood: ", likelihood)
    ...
    if model == "densenet":
        ...
        net = PreActResNet18(prior_w=weight_prior, loc_w=weight_loc, std_w=weight_scale,
                            prior_b=bias_prior, loc_b=bias_loc, std_b=bias_scale, scaling_fn=scaling_fn,
                            bn=batchnorm, softmax_temp=softmax_temp, weight_prior_params=weight_prior_params,
        ...
        net = ResNet(prior_w=weight_prior, loc_w=weight_loc, std_w=weight_scale, depth=20,
                     prior_b=bias_prior, loc_b=bias_loc, std_b=bias_scale, scaling_fn=scaling_fn,
                     bn=batchnorm, softmax_temp=softmax_temp, weight_prior_params=weight_prior_params,
                     bias_prior_params=bias_prior_params, label_smoothing=label_smoothing,
                     likelihood=likelihood, consistency_std=consistency_std).to(x_train)
