...
class SGLDRunner:
    ...
    def run(self, progressbar=False):
            ...
            for epoch in epochs:
                ...
                for i, (x, y) in enumerate(self.dataloader):
                    ...
                    if isinstance(y, list):
                        y = [part.to(self._params[0].device).detach() for part in y]
                    else:
                        y = y.to(self._params[0].device).detach()

                    loss, acc, delta_energy = self.step(
                        step, x.to(self._params[0].device).detach(), y,
        ...
        if isinstance(y, list):
            y = [part.to(self._params[0].device) for part in y]
        else:
            y = y.to(self._params[0].device)
        self.step(step+1,
