...
__all__ = (
        'RegressionModel', 'RaoBRegressionModel', 'ClassificationModel', 'AbstractModel',
        'ConsistencyModel',
)
...
class DirichletClassificationModel(ClassificationModel):
    def __init__(self, net: nn.Module, softmax_temp: Union[float, torch.Tensor] = 0.01):
        super().__init__(net=net, softmax_temp=softmax_temp)
        
    def likelihood_dist(self, f: torch.Tensor):
        return DirichetGaussian(mean=f, alpha_epsilon=self.softmax_temp)
    

class DirichletGaussian(torch.distributions.Normal):
    def __init__(self, mean, alpha_epsilon):
        super().__init__(mean, torch.ones_like(mean), validate_args=False)
        self.alpha_epsilon = alpha_epsilon
        
    def log_prob(self, y):
        # first convert y to one hot
        # num_classes = int(y.max() + 1)
        num_classes = self.mean.shape[-1]
        # onehot_y = torch.nn.functional.one_hot(y)
        alpha = self.alpha_epsilon * torch.ones(y.shape[-1], num_classes, device=y.device, dtype=y.dtype)
        alpha[torch.arange(len(y)), y] = alpha[torch.arange(len(y)), y] + 1.0
        sigma2_i = torch.log(1 / alpha + 1.0)

        # y = log(alpha) - 0.5 * sigma^2
        transformed_targets = alpha.log() - 0.5 * sigma2_i
        self.scale = sigma2_i.pow(0.5)
        return super().log_prob(transformed_targets)
    

class SmoothedCategorical(torch.distributions.Categorical):
    def __init__(self, probs=None, logits=None, smoothing=0.0, validate_args=None):
        self.smoothing = smoothing

    def log_prob(self, y):
        return torch.nn.functional.cross_entropy(self.logits, y, label_smoothing=self.smoothing)


class SmoothedClassificationModel(ClassificationModel):
    """Model that enables some amount of label smoothing.
    TODO: also implement as a regression likelihood
    Arguments:
        net: modules to evaluate to get the latent function
        softmax_temp: amount of labels moothing
    """

    def __init__(self, net: nn.Module, smoothing: Union[float, torch.Tensor]=1.):
        super().__init__(net, softmax_temp=1.)
        self.smoothing = smoothing

    def likelihood_dist(self, f: torch.Tensor):
        return SmoothedCategorical(logits=f, smoothing=self.smoothing)

class ColdCategorical(torch.distributions.Categorical):
    def __init__(self, probs=None, logits=None, temperature=1.0, validate_args=None):
        super().__init__(probs=probs, logits=logits, validate_args=validate_args)
        self.temperature = temperature


    def log_prob(self, y):
        res = super().log_prob(y)
        return res / self.temperature

class ColdClassificationModel(ClassificationModel):
    def __init__(self, net: nn.Module, softmax_temp: Union[float, torch.Tensor]=1.0):
        super().__init__(net, softmax_temp=softmax_temp)

    def likelihood_dist(self, f: torch.Tensor):
        return ColdCategorical(logits=f, temperature=self.softmax_temp)


class ConsistencyModel(ClassificationModel):
    def __init__(self, net: nn.Module, consistency_std: Union[float, torch.Tensor]=1.0):
        # softmax_temp is the prior std for the consistency term here
        super().__init__(net, softmax_temp=consistency_std)

    def likelihood_dist(self, f: torch.Tensor):
        return ConsistencyLikelihood(logits=f, sigma=self.softmax_temp, net=self.net)

class ConsistencyLikelihood(torch.distributions.Categorical):
    def __init__(self, net: nn.Module, probs=None, logits=None, sigma=1.0, validate_args=None):
        super().__init__(probs=probs, logits=logits, validate_args=False)
        self.net = net
        self.sigma = sigma
    ...
    def log_prob(self, y):
        if isinstance(y, list):
            targets, inputs = y
            # classification_loss = super().log_prob(targets)
            
            # TODO: some detaching here?
            # print(inputs.shape, "here")
            forwards_pass_list = []
            for i in range(inputs.shape[1]):
                out = self.net(inputs[:, i])
                forwards_pass_list.append(out.unsqueeze(1))
            forwards_pass = torch.cat(forwards_pass_list, dim=-2).permute(1, 0, 2)
            # print(forwards_pass.shape)
            # should be K x batch_shape x NC outputs
            # dist = torch.distributions.Normal(self.logits, self.sigma).log_prob(forwards_pass)
            # consistency_loss = dist.mean(-1).sum(0)
            augmented_dist = torch.distributions.Categorical(logits=forwards_pass)
            consistency_loss = -torch.distributions.kl.kl_divergence(self, augmented_dist)
            consistency_loss = consistency_loss / self.sigma
            classification_loss = augmented_dist.log_prob(targets)

            # print(consistency_loss.mean(), classification_loss.mean())
            # average over the mean directions
            return (consistency_loss + classification_loss).mean(0)
        else:
