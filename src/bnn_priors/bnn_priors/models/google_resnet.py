...
from .base import ClassificationModel, ConsistencyModel, SmoothedClassificationModel, ColdClassificationModel
from .. import prior
...
def ResNet(softmax_temp=1., depth=20, num_classes=10,
           ...
           conv_prior_w=prior.Normal, label_smoothing=0., likelihood="std", consistency_std=100):
    print(softmax_temp, label_smoothing, likelihood, "output")
    ...
    if label_smoothing == 0.0:
        if likelihood == True or likelihood == "cold":
            return ColdClassificationModel(nn.Sequential(*layers), softmax_temp=softmax_temp)
        elif likelihood == False or likelihood == "std":
            return ClassificationModel(nn.Sequential(*layers), softmax_temp=softmax_temp)
        elif likelihood == "consistency":
            return ConsistencyModel(nn.Sequential(*layers), consistency_std=consistency_std)
    else:
        return SmoothedClassificationModel(nn.Sequential(*layers), smoothing=label_smoothing)
